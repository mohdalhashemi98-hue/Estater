...
const COLORS = ['#c96442', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#14B8A6', '#F97316', '#06B6D4', '#84CC16'];
...
export default function ExpenseSummaryCharts({ summary }: { summary: ExpenseSummary }) {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-5">
      {/* Category pie chart */}
      <div className="bg-white rounded-xl border border-surface-border p-5 chart-reveal">
        <h3 className="text-sm font-medium text-text-primary mb-4">By Category</h3>
        {pieData.length > 0 ? (
      ...
      <div className="bg-white rounded-xl border border-surface-border p-5 chart-reveal" style={{ animationDelay: '0.15s' }}>
        ...
        {summary.monthly_trend.length > 0 ? (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={summary.monthly_trend}>
              ...
              <YAxis tick={{ fontSize: 11, fill: '#83827d' }} axisLine={false} tickLine={false} />
              ...
              <Bar dataKey="total" fill="#c96442" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        ) : (
          <p className="text-sm text-text-muted py-6 text-center">No data</p>

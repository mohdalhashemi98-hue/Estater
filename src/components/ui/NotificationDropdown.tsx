  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="relative text-text-muted hover:text-text-secondary transition-colors p-1"
      >
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-[360px] bg-white rounded-xl border border-surface-border shadow-xl z-50 animate-scale-in origin-top-right">
                  ...
                  <div>
                    ...
                    {overdue.slice(0, 5).map(p => (
                      <Link
                        ...
                        className="flex items-start gap-3 px-4 py-2.5 hover:bg-surface transition-colors"
                      >
                        <div className="w-7 h-7 rounded-lg bg-red-50 flex items-center justify-center shrink-0 mt-0.5">
                          <AlertTriangle className="w-3.5 h-3.5 text-red-500" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-text-primary truncate">{p.tenant_name}</p>
                          <p className="text-xs text-text-muted truncate">{p.property_name} — {p.unit_number}</p>
                        </div>
                  ...
                  <div>
                    ...
                    {expiring.slice(0, 5).map(c => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-text-primary truncate">{c.tenant_name}</p>
                          <p className="text-xs text-text-muted truncate">{c.property_name} — {c.unit_number}</p>
                        </div>
                        <div className="text-right shrink-0">

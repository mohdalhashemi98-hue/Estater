    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3 animate-fade-in">
        <div className="flex items-center gap-2">
          <select
            className="border border-surface-border rounded-lg px-3 py-2 text-sm bg-white focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20 outline-none"
            value={filter}
          ...
          <span className="text-sm text-text-muted">{contracts.length} contracts</span>
        </div>
        <div className="flex items-center gap-2">
          <input
            ref={fileInputRef}
      ...
      {uploadMutation.isPending && (
        <div className="bg-white rounded-xl border border-surface-border p-8 shadow-sm modal-content">
          <div className="flex flex-col items-center justify-center">
            ...
            <p className="mt-4 text-sm font-medium text-text-primary">AI is analyzing your contract...</p>
            <p className="text-xs text-text-muted mt-1">Extracting tenant, property, and payment details</p>
          </div>
        </div>
      )}
      ...
      {showAiReview && aiAnalysis && (
        <div className="bg-white rounded-xl border border-surface-border p-6 shadow-sm animate-slide-up space-y-5">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-xl p-4 space-y-2">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-sm font-semibold text-text-primary">Tenant</h4>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  <input className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.tenant.first_name + ' ' + aiAnalysis.tenant.last_name} onChange={e => {
                ...
                <div>
                  ...
                  <input className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.tenant.phone} onChange={e => setAiAnalysis({ ...aiAnalysis, tenant: { ...aiAnalysis.tenant, phone: e.target.value } })} />
                </div>
                <div>
                  <p className="text-text-muted text-xs">Email</p>
                  <input className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.tenant.email || ''} onChange={e => setAiAnalysis({ ...aiAnalysis, tenant: { ...aiAnalysis.tenant, email: e.target.value || null } })} />
                </div>
                <div>
                  <p className="text-text-muted text-xs">ID Number</p>
                  <input className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.tenant.id_number || ''} onChange={e => setAiAnalysis({ ...aiAnalysis, tenant: { ...aiAnalysis.tenant, id_number: e.target.value || null } })} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-4 space-y-2">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-sm font-semibold text-text-primary">Property</h4>
                <span className="text-xs bg-accent-100 text-accent-700 px-2 py-0.5 rounded-full font-medium">New</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <p className="text-text-muted text-xs">Name</p>
                  <input className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.property.name} onChange={e => setAiAnalysis({ ...aiAnalysis, property: { ...aiAnalysis.property, name: e.target.value } })} />
                </div>
                <div>
                  <p className="text-text-muted text-xs">Emirate</p>
                  <select className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.property.emirate} onChange={e => setAiAnalysis({ ...aiAnalysis, property: { ...aiAnalysis.property, emirate: e.target.value } })}>
                    {UAE_EMIRATES.map(em => <option key={em} value={em}>{em}</option>)}
                  </select>
                </div>
                <div>
                  <p className="text-text-muted text-xs">City / Area</p>
                  <input className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.property.city || ''} onChange={e => setAiAnalysis({ ...aiAnalysis, property: { ...aiAnalysis.property, city: e.target.value || null } })} />
                </div>
                <div>
                  <p className="text-text-muted text-xs">Unit #</p>
                  <input className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.unit.unit_number} onChange={e => setAiAnalysis({ ...aiAnalysis, unit: { ...aiAnalysis.unit, unit_number: e.target.value } })} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-4 space-y-2 md:col-span-2">
              ...
              <div className="grid md:grid-cols-4 gap-3 text-sm">
                <div>
                  ...
                  <input type="date" className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.contract.start_date} onChange={e => setAiAnalysis({ ...aiAnalysis, contract: { ...aiAnalysis.contract, start_date: e.target.value } })} />
                </div>
                <div>
                  <p className="text-text-muted text-xs">End Date</p>
                  <input type="date" className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.contract.end_date} onChange={e => setAiAnalysis({ ...aiAnalysis, contract: { ...aiAnalysis.contract, end_date: e.target.value } })} />
                </div>
                <div>
                  <p className="text-text-muted text-xs">Rent Amount (per period)</p>
                  <input type="number" className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.contract.rent_amount} onChange={e => setAiAnalysis({ ...aiAnalysis, contract: { ...aiAnalysis.contract, rent_amount: Number(e.target.value) } })} />
                </div>
                <div>
                  <p className="text-text-muted text-xs">Payment Frequency</p>
                  <select className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.contract.payment_frequency} onChange={e => setAiAnalysis({ ...aiAnalysis, contract: { ...aiAnalysis.contract, payment_frequency: e.target.value } })}>
                ...
                <div>
                  <p className="text-text-muted text-xs">Total Payments / Cheques</p>
                  <input type="number" className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.contract.total_payments} onChange={e => setAiAnalysis({ ...aiAnalysis, contract: { ...aiAnalysis.contract, total_payments: Number(e.target.value) } })} />
                </div>
                <div>
                  <p className="text-text-muted text-xs">Security Deposit</p>
                  <input type="number" className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.contract.deposit_amount || ''} onChange={e => setAiAnalysis({ ...aiAnalysis, contract: { ...aiAnalysis.contract, deposit_amount: e.target.value ? Number(e.target.value) : null } })} placeholder="Optional" />
                </div>
                <div className="md:col-span-2">
                  <p className="text-text-muted text-xs">Notes</p>
                  <input className="w-full border border-surface-border rounded-lg px-3 py-1.5 text-sm bg-white" value={aiAnalysis.contract.notes || ''} onChange={e => setAiAnalysis({ ...aiAnalysis, contract: { ...aiAnalysis.contract, notes: e.target.value || null } })} />
                </div>
              </div>
            </div>
          </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-surface-border p-6 shadow-sm modal-content">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={() => { setShowForm(false); setSelectedPropertyId(null); }} className="px-4 py-2 rounded-lg text-sm text-text-muted hover:bg-surface transition-colors">Cancel</button>
          </div>
        </div>
      )}
              ...
              {contracts.map((c, i) => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-surface transition-colors animate-fade-in" style={{ animationDelay: `${Math.min(i * 50, 300)}ms` }}>
                    ...
                    <td className="px-4 py-3 text-text-muted">{c.property_name} - {c.unit_number}</td>
                    <td className="px-4 py-3 text-text-muted text-xs">{formatDate(c.start_date)} - {formatDate(c.end_date)}</td>
                    <td className="px-4 py-3 font-medium text-text-primary">{formatCurrency(c.rent_amount)}/{frequencyLabel(c.payment_frequency).toLowerCase()}</td>
                    <td className="px-4 py-3 text-text-muted">{c.total_payments} checks</td>
                    <td className="px-4 py-3">
                      {(c.file_count ?? 0) > 0 ? (
                    ...
                    <td className="px-4 py-3">
                      {c.status === 'active' && (
                        <span className={`text-xs font-medium ${days <= 30 ? 'text-red-600' : days <= 60 ? 'text-amber-600' : 'text-text-muted'}`}>

    ...
    <div className="space-y-6 max-w-[1200px]">
      <div className="flex items-center justify-between animate-fade-in">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Track and manage property expenses</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 bg-white rounded-xl border border-gray-200 p-4 animate-fade-in animate-stagger-1">
        ...
        <input type="date" value={filters.from_date} onChange={e => setFilters(f => ({ ...f, from_date: e.target.value }))}
          className="text-sm border border-gray-200 rounded-lg px-3 py-1.5" placeholder="From" />
        <input type="date" value={filters.to_date} onChange={e => setFilters(f => ({ ...f, to_date: e.target.value }))}
          className="text-sm border border-gray-200 rounded-lg px-3 py-1.5" placeholder="To" />
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={stat.label} className={`bg-white rounded-xl border border-gray-200 p-4 card-hover animate-fade-in animate-stagger-${i + 2}`}>
              <p className="text-xs text-gray-400 font-medium">{stat.label}</p>
              <p className="text-xl font-semibold text-gray-900 mt-1 stat-value">{stat.value}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/20 z-50 flex items-start justify-center pt-20 modal-backdrop">
          <div className="bg-white rounded-xl border border-gray-200 shadow-xl w-full max-w-lg p-6 modal-content">
      ...
      {/* Table */}
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden animate-fade-in animate-stagger-6">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Date</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Property</th>
                ...
                <th className="text-center px-4 py-3 text-xs font-medium text-gray-500">Receipt</th>
                <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">Actions</th>
              </tr>
            </thead>
            <tbody>
              {expenses.map((exp) => (
                <tr key={exp.id} className="border-b border-gray-50 row-hover table-row-enter">
                  <td className="px-4 py-3 text-gray-700">{formatDate(exp.expense_date)}</td>
                  <td className="px-4 py-3 text-gray-700">{exp.property_name}{exp.unit_number ? ` — ${exp.unit_number}` : ''}</td>
                  <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">{categoryLabel(exp.category)}</span></td>
                  <td className="px-4 py-3 text-gray-500">{exp.vendor_name || '-'}</td>
                  <td className="px-4 py-3 text-right font-medium text-gray-900 tabular-nums">{formatCurrency(exp.amount, exp.currency)}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => { setEditing(exp); setShowForm(true); }} className="p-1 text-gray-400 hover:text-gray-600"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => { if (confirm('Delete this expense?')) deleteMut.mutate(exp.id); }} className="p-1 text-gray-400 hover:text-red-600"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}

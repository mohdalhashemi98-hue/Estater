...
export default function MortgageDetail() {
  ...
  return (
    <div className="space-y-6">
      <Link to={`/properties/${mortgage.property_id}`} className="inline-flex items-center gap-1 text-sm text-text-muted hover:text-text-primary transition-colors">
        ...
        {editing ? (
          <>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-text-primary">Edit Mortgage</h2>
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-text-primary">{mortgage.lender_name}</h2>
                  <p className="text-sm text-text-muted">{mortgage.property_name} &middot; {mortgage.loan_type} rate at {mortgage.interest_rate}%</p>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <button onClick={() => { if (confirm('Delete this mortgage and all its payments?')) deleteMutation.mutate(); }} className="p-2 text-text-muted hover:text-danger-600 hover:bg-danger-50 rounded-lg transition-colors" title="Delete"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-surface rounded-lg p-3">
                ...
                <p className="text-lg font-bold text-text-primary">{formatCurrency(mortgage.loan_amount)}</p>
              </div>
              <div className="bg-surface rounded-lg p-3">
                <p className="text-xs text-text-muted">Monthly Payment</p>
                <p className="text-lg font-bold text-text-primary">{formatCurrency(mortgage.monthly_payment)}</p>
              </div>
              <div className="bg-surface rounded-lg p-3">
                <p className="text-xs text-text-muted">Remaining Balance</p>
                <p className="text-lg font-bold text-text-primary">{formatCurrency(mortgage.remaining_balance ?? mortgage.loan_amount)}</p>
              </div>
              <div className="bg-surface rounded-lg p-3">
                <p className="text-xs text-text-muted">Term</p>
                <p className="text-lg font-bold text-text-primary">{mortgage.term_months} months ({Math.round(mortgage.term_months / 12)} yrs)</p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-surface-border p-4 shadow-sm animate-fade-in animate-stagger-1">
          <p className="text-sm text-text-muted">Principal Paid</p>
        ...
        <div className="bg-white rounded-xl border border-surface-border p-4 shadow-sm animate-fade-in animate-stagger-2">
          <p className="text-sm text-text-muted">Interest Paid</p>
        ...
        <div className="bg-white rounded-xl border border-surface-border p-4 shadow-sm animate-fade-in animate-stagger-3">
          ...
          <p className="text-xl font-bold text-text-primary">{formatCurrency(totalPrincipalPaid + totalInterestPaid)}</p>
        </div>
      </div>
      ...
      {payments.length > 0 && (
        <div className="bg-white rounded-xl border border-surface-border p-6 shadow-sm">
          <h3 className="font-semibold text-text-primary mb-4">Amortization Schedule</h3>
        ...
        <div className="max-h-[500px] overflow-y-auto overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-surface-border">
              {payments.map((p, i) => (
                <tr key={p.id} className={`transition-colors animate-fade-in-fast ${p.status === 'paid' ? 'bg-success-50/30' : 'hover:bg-surface'}`} style={{ animationDelay: `${Math.min(i * 20, 300)}ms` }}>
                  <td className="px-4 py-2.5 text-text-muted">{p.payment_number}</td>
                  <td className="px-4 py-2.5 text-text-primary">{formatDate(p.due_date)}</td>
                  <td className="px-4 py-2.5 text-right text-text-primary">{formatCurrency(p.principal)}</td>
                  <td className="px-4 py-2.5 text-right text-text-muted">{formatCurrency(p.interest)}</td>
                  <td className="px-4 py-2.5 text-right font-medium text-text-primary">{formatCurrency(p.total_amount)}</td>
                  <td className="px-4 py-2.5 text-right text-text-muted">{formatCurrency(p.remaining_balance)}</td>

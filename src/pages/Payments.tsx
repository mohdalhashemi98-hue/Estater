    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3 animate-fade-in">
        <div className="flex items-center gap-2">
          <select className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20 outline-none" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4 card-hover animate-fade-in animate-stagger-1">
          <p className="text-sm text-gray-500">Total Due</p>
          <p className="text-xl font-bold text-gray-900 stat-value">{formatCurrency(summary.totalAmount)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 card-hover animate-fade-in animate-stagger-2">
          <p className="text-sm text-gray-500">Collected</p>
          <p className="text-xl font-bold text-emerald-600 stat-value">{formatCurrency(summary.paidAmount)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 card-hover animate-fade-in animate-stagger-3">
          <p className="text-sm text-gray-500">Pending</p>
          <p className="text-xl font-bold text-amber-600 stat-value">{summary.pending}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 card-hover animate-fade-in animate-stagger-4">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-gray-500">Due Date</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-gray-500">Amount</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-gray-500">Status</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-gray-500">Paid</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-gray-500">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {payments.map((p, i) => (
                <tr key={p.id} className="hover:bg-gray-50 transition-colors animate-fade-in" style={{ animationDelay: `${Math.min(i * 30, 300)}ms` }}>
                  <td className="px-4 py-3">
                    <Link to={`/contracts/${p.contract_id}`} className="font-medium text-accent-600 hover:underline">{p.tenant_name}</Link>
                  </td>
                  <td className="px-4 py-3 text-gray-500">{p.property_name} - {p.unit_number}</td>
                  ...
                  <td className="px-4 py-3">
                    {(p.status === 'pending' || p.status === 'overdue') && (
                      <>
                        {payingId === p.id ? (
                          <div className="flex items-center gap-2">
                            <select className="border border-gray-200 rounded-lg px-3 py-2 text-xs bg-white focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20" value={payMethod} onChange={e => setPayMethod(e.target.value)}>
                              <option value="check">Check</option>
                              <option value="bank_transfer">Transfer</option>
                              <option value="cash">Cash</option>
                              <option value="other">Other</option>
                            </select>
                            <input className="border border-gray-200 rounded-lg px-3 py-2 text-xs w-20 bg-white focus:border-accent-500 focus:ring-2 focus:ring-accent-500/20" placeholder="Ref #" value={payRef} onChange={e => setPayRef(e.target.value)} />
                            ...
                            <button onClick={() => setPayingId(null)} className="text-gray-500 text-xs">X</button>
                          </div>
                        ) : (

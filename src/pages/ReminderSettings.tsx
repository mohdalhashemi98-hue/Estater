  ...
  return (
    <div className="space-y-6 max-w-[800px]">
      <div className="animate-fade-in">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Configure automated reminders for payments and contracts</p>
      </div>

      {/* Settings table */}
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden animate-fade-in animate-stagger-1">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Type</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Days Before</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Method</th>
              <th className="text-center px-4 py-3 text-xs font-medium text-gray-500">Enabled</th>
              <th className="text-right px-4 py-3 text-xs font-medium text-gray-500">Actions</th>
            </tr>
          </thead>
          <tbody>
            {settings.map((s) => (
              <tr key={s.id} className="border-b border-gray-50 table-row-enter">
                <td className="px-4 py-3 text-gray-700 font-medium">{typeLabel[s.reminder_type] || s.reminder_type}</td>
                <td className="px-4 py-3 text-gray-600">{s.days_before} days</td>
                <td className="px-4 py-3 text-gray-500 capitalize">{s.notification_method}</td>
                <td className="px-4 py-3 text-center">
                  <button onClick={() => toggleMut.mutate(s)}
                    className={`w-10 h-5 rounded-full transition-colors relative ${s.enabled ? 'bg-accent-600' : 'bg-gray-300'}`}>
                ...
                <td className="px-4 py-3 text-right">
                  <button onClick={() => { if (confirm('Delete?')) deleteMut.mutate(s.id); }} className="text-gray-400 hover:text-red-600">
        ...
        <div className="flex items-center gap-3 p-4 border-t border-gray-100 bg-gray-50">
          ...
          <input type="number" min="1" value={newDays} onChange={e => setNewDays(Number(e.target.value))}
            className="w-20 text-sm border border-gray-200 rounded-lg px-3 py-1.5" placeholder="Days" />
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 animate-fade-in animate-stagger-2">
        <h3 className="text-sm font-medium text-gray-900 mb-4">Recent Reminder Logs</h3>
        {logs.length > 0 ? (
          <div className="space-y-2">
            {logs.map(log => (
              <div key={log.id} className="flex items-center justify-between text-xs py-2 border-b border-gray-50">
                ...
                <span className="text-gray-400">{new Date(log.triggered_at).toLocaleString()}</span>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-sm text-gray-400 text-center py-4">No reminders sent yet</p>
        )}
      </div>
    </div>
  );
